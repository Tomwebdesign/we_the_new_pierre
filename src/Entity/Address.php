...
class Address
{
    ...
    public function addDefaultUser(User $defaultUser): static
    {
        if (!$this->defaultUsers->contains($defaultUser)) {
            $this->defaultUsers->add($defaultUser);
            $defaultUser->setDefaultAddress($this);
        }

        return $this;
    }

    public function removeDefaultUser(User $defaultUser): static
    {
        if ($this->defaultUsers->removeElement($defaultUser)) {
            // set the owning side to null (unless already changed)
            if ($defaultUser->getDefaultAddress() === $this) {
                $defaultUser->setDefaultAddress(null);
            }
        }
